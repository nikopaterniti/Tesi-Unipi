...
# Funzione per calcolare il BERT score
...
# Funzione per segmentare la frase e calcolare il BLEU, SacreBLEU, ROUGE e BERT score per ciascun segmento
def segment_and_calculate_scores(reference, answer):
    ...
    # Calcola il BERT Score per ciascun segmento
    bert_scores = [calculate_bert_score(reference, sentence) for sentence in sentences]
    max_bert_scores = {
        'precision': max([score[0] for score in bert_scores]) if bert_scores else 0,
        'recall': max([score[1] for score in bert_scores]) if bert_scores else 0,
        'f1': max([score[2] for score in bert_scores]) if bert_scores else 0,
    }

    return max_bleu_score, max_sacrebleu_score, max_rouge_scores, max_bert_scores
...
# Calcola i BLEU, SacreBLEU, ROUGE e BERT score per ogni risposta segmentata
...
df['max_bert_precision'] = results.apply(lambda x: float(x[3]['precision']))
df['max_bert_recall'] = results.apply(lambda x: float(x[3]['recall']))
df['max_bert_f1'] = results.apply(lambda x: float(x[3]['f1']))

# Filtra le colonne per visualizzare solo gli score
scores_df = df[['max_bleu_score', 'max_sacrebleu_score', 'max_rouge1_score', 'max_rouge2_score', 'max_rougeL_score', 'max_bert_precision', 'max_bert_recall', 'max_bert_f1']]
...
average_max_bert_precision = scores_df['max_bert_precision'].mean()
average_max_bert_recall = scores_df['max_bert_recall'].mean()
average_max_bert_f1 = scores_df['max_bert_f1'].mean()
...
print(f"Average Max BERT Precision: {average_max_bert_precision:.4f}")
print(f"Average Max BERT Recall: {average_max_bert_recall:.4f}")
print(f"Average Max BERT F1 Score: {average_max_bert_f1:.4f}")
